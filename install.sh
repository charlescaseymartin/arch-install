...
arch-chroot /mnt sh -c '$(curl -fsSL https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh | sed -e "s.#!/bin/sh.#!/usr/bin/env sh.")'
arch-chroot /mnt sh -c 'git clone https://github.com/charlescaseymartin/archlinux-moded-dotfiles.git'
arch-chroot /mnt sh -c 'cd archlinux-moded-dotfiles'
arch-chroot /mnt sh -c 'install.sh -i'
